  ...
  return (
    <section id="project" ref={ref} style={{ padding: '6rem 3rem', borderBottom: '0.5px solid #1a1a1a' }}>
      <div className="fade" style={{ opacity: 0, transform: 'translateY(30px)', transition: 'opacity 0.7s ease, transform 0.7s ease' }}>
        <h2 style={{ fontSize: 'clamp(3rem, 8vw, 7rem)', fontWeight: 800, lineHeight: 0.95, letterSpacing: '-2px', textTransform: 'uppercase' }}>
          <div>SELECTED</div>
          <div style={{ color: '#333' }}>WORKS.</div>
        </h2>
        <p style={{ fontSize: '13px', letterSpacing: '2px', color: '#555', textTransform: 'uppercase', marginTop: '2rem', maxWidth: '480px', lineHeight: 1.8 }}>
          A curated collection of my most impactful projects, bridging functionality with immersive digital experiences.
        </p>
      </div>
        ...
        {projects.map((project, i) => (
          ...
          >
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '1.5rem' }}>
              <span style={{ fontSize: '11px', color: '#555', letterSpacing: '1px', textTransform: 'uppercase' }}>{project.tech}</span>
              <span style={{ fontSize: '11px', color: '#444' }}>{'—'}</span>
              <span style={{ fontSize: '11px', color: '#444' }}>{project.year}</span>
              <div style={{ width: '32px', height: '32px', border: '0.5px solid #333', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#555', fontSize: '14px' }}>{'↗'}</div>
            </div>
          </div>
        ))}
      </div>
